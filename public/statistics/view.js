...
function drawChart(days) {
    ...
    svg.setAttribute('aria-label', 'צפיות יומיות. הנתונים המלאים זמינים בטבלה שמתחת לגרף.');
    ...
    days.filter(day => day.views > 0).forEach(day => {
        const point = add('circle', { cx: x(day), cy: y(day.views), r: 4, fill: '#2563eb', tabindex: 0 });
        const title = document.createElementNS(ns, 'title');
        title.textContent = `${dateLabel(day.date)}: ${number(day.views)} צפיות`;
...
export class StatisticsView {
    ...
    render({ totalViews, articleCount, publications, averageViews, chartDays, statuses, ranking }) {
        ...
        drawChart(chartDays);
        $('#daily-table').replaceChildren(...(chartDays.length ? chartDays.map(day => cellRow([dateLabel(day.date), number(day.views)])) : [cellRow(['אין נתוני צפייה', '—'])]));
